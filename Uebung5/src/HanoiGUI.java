...
public class HanoiGUI {
	...
	private final int DELAY_PER_STEP = 10; // ms
	...
	private class HanoiCanvas extends Canvas {
		...
		private void drawPile(Graphics graphics, int pileNo) {
			...
			graphics.fillRect(PILE_DISTANCE * (pileNo + 1) - PILE_THICKNESS / 2, TOP_GAP, PILE_THICKNESS, PILE_HEIGHT); 
		...
		private void drawDisks(Graphics graphics, int pileNo) {
			...
			for (int i = 0; i < disks.length; i++) {
				...
				int top = TOP_GAP + PILE_HEIGHT - DISK_THICKNESS * (i + 1) - DISK_GAP * i;
